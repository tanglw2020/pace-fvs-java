...
public class ReductionRoot {
	...
	public static void reduce(Graph g) {
		...
		if (DEBUG)
			System.err.println("reduction finished");
	}
	...
	static boolean selfLoop(Graph g) {
		...
		int newN = g.n();
		if (DEBUG && oldN != newN)
			System.err.printf("selfLoop: %d -> %d%n", oldN, newN);
		return oldN != newN;
	}
	...
	static boolean deg2(Graph g) {
		...
		int newN = g.n();
		if (DEBUG && oldN != newN)
			System.err.printf("deg2: %d -> %d%n", oldN, newN);
		return oldN != newN;
	}
	...
	static boolean sameNeighbor(Graph g) {
		...
		for (int v = 0; v < g.n; v++)
			if (g.used[v] == 0) {
				...
				if (a == null)
					map.put(g.adj[v], a = new IntArray());
				a.add(v);
			}
		...
		int newN = g.n();
		if (DEBUG && oldN != newN)
			System.err.printf("sameN: %d -> %d%n", oldN, newN);
		return oldN != newN;
	}
	...
	static boolean dominate(Graph g) {
		int oldN = g.n();
		for (int v = 0; v < g.n; v++) {
			loop: for (int i = 0; i + 1 < g.adj[v].length; i++)
				if (g.adj[v][i] == g.adj[v][i + 1]) {
					int u = g.adj[v][i++];
					if (g.adj[v].length >= g.adj[u].length) {
						for (int j = 0, p = 0; j < g.adj[u].length; j++)
							if (g.adj[u][j] != v) {
								while (p < g.adj[v].length && (g.adj[u][j] > g.adj[v][p] || g.adj[v][p] == u))
									p++;
								if (p >= g.adj[v].length || g.adj[u][j] < g.adj[v][p])
									continue loop;
								p++;
							}
						g.setS(v);
		...
		int newN = g.n();
		if (DEBUG && oldN != newN)
			System.err.printf("dominate: %d -> %d%n", oldN, newN);
		return oldN != newN;
	}
	...
	static boolean bridge(Graph g) {
		...
		int newN = g.n();
		if (DEBUG && reduced)
			System.err.printf("bridge: %d -> %d%n", oldN, newN);
		return reduced;
	}
