...
public class Reduction {
	...
	static boolean deg2Ex(Graph g) {
		...
		for (int i = 0; i < g.n; i++)
			if (g.used[i] == 'F' && g.adj[i].length > 0) {
				for (int v : g.adj[i])
					adjS[v] = true;
			}
		int qs = 0, qt = 0;
		loop: for (int v = 0; v < g.n; v++)
			if (g.used[v] == 0) {
				...
				while (qs < qt) {
					...
					loop2: for (int i = 0; i < d; i++)
						if ((F >> i & 1) == 0 && !checked[F | (1 << i)]) {
							int count = 0;
							for (int j = 0; j < d && count <= 1; j++)
								if ((F >> j & 1) != 0 && h[i][j] > 0) {
									if (adjS[N[i]] && adjS[N[j]])
										continue loop2;
									count += h[i][j];
								}
							if (count <= 1) {
								if (size + 1 >= 4 || independent && count == 0)
									continue loop;
								checked[F | 1 << i] = true;
								que[qt++] = F | 1 << i;
							}
	...
	static boolean degLB(Graph g, int ub) {
		...
		for (int i = 0; i < g.n; i++)
			if (g.adj[i].length > 0) {
				...
				m += g.adj[i].length;
				if (g.used[i] != 'F')
					deg[p++] = g.adj[i].length;
			}
		m /= 2;
		if (p == 0 || g.k + 1 >= ub || g.k + p < ub)
			return false;
		sort(deg, 0, p);
		int sum = 0, k = 1;
		for (; k + g.k < ub; k++) {
		...
		int s = -1;
		for (int i = 0; i < g.n; i++)
			if (g.adj[i].length > 0 && g.used[i] == 'F')
				s = i;
		if (s >= 0) {
			for (int i : g.adj[s])
				adjF[i] = true;
		...
		for (int i = 0; i < g.n; i++)
			if (adjF[i] && g.adj[i].length < deg[p - k + 1] && g.used[i] != 'F') {
	...
	static boolean dominate(Graph g) {
		...
		boolean[] adjS = new boolean[g.n];
		for (int s = 0; s < g.n; s++)
			if (g.used[s] == 'F' && g.adj[s].length > 0) {
				for (int v : g.adj[s])
					adjS[v] = true;
			}
		for (int v = 0; v < g.n; v++)
