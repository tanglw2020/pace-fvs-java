...
public class Graph {
	...
	public int N(int v, int[] list) {
		...
		for (int i = 0; i < adj[v].length; i++) {
			list[p++] = adj[v][i];
			if (i + 1 < adj[v].length && adj[v][i + 1] == adj[v][i])
				i++;
		}
		return p;
	}
	...
	public void test() {
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < adj[i].length - 1; j++)
				Debug.check(adj[i][j] <= adj[i][j + 1]);
			for (int j : adj[i]) {
				Debug.check(j != i);
				int d = hasEdge(i, j);
				Debug.check(d <= 2);
